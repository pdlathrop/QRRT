...
%% Description:
% QQRT function takes as inputs an environment and initial condition and
% outputs a tree of states spanning the environment once a node has been
% placed near a goal state
%% Inputs: 
% grid: Boolean array (shape = (L,L)), obstalce environment, represented as 2D array with 1's for
...
while(~finished)
    ...
    %do additional check to account for measurement error
    oracle_count = oracle_count + 1;
    ...
    function parent_ind = findParent(x,node_list)
        ...
        % Date last edited: 4/6/23
        %% Description:
        % Inline function finds the nearest parent in node_list (based on Manhattan norm) of x
        %% Inputs:
        ...
        for j = 1:length(node_list(:,1)) %iterate over node_list
            temp = quick1Norm(x,node_list(j,:),2);
            if(temp < min_dist), min_dist = temp; parent_ind = j; end
        end
    end
